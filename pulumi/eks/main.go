// The source code is based on these samples:
// https://github.com/pulumi/examples/tree/master/aws-go-eks
// https://www.pulumi.com/registry/packages/aws/api-docs/eks/podidentityassociation/
package main
...
func main() {
	pulumi.Run(func(ctx *pulumi.Context) error {
		...
		bucket, err := s3.NewBucket(ctx, "pod-identities-bucket-pulumi", nil)
		if err != nil {
			return err
		}

		// Export the name of the bucket
		ctx.Export("pod-identities-bucket-pulumi", bucket.ID())
		...
		// Create EKS Cluster
		eksCluster, err := eks.NewCluster(ctx, "pod-identities-demo-pulumi", &eks.ClusterArgs{
		...
		_, err = eks.NewAddon(ctx, "eks-pod-identity-agent", &eks.AddonArgs{
		...
		example, err := iam.NewRole(ctx, "pod-identities-role-pulumi", &iam.RoleArgs{
		...
		appLabels = pulumi.StringMap{
			"app": pulumi.String("pod-identities-demo"),
		}
		_, err = appsv1.NewDeployment(ctx, "pod-identities-demo-pod", &appsv1.DeploymentArgs{
			Metadata: &metav1.ObjectMetaArgs{
				Namespace: pulumi.String("default"),
			},
			Spec: appsv1.DeploymentSpecArgs{
					...
					Spec: &corev1.PodSpecArgs{
						Containers: corev1.ContainerArray{
							corev1.ContainerArgs{
								...
								Image: pulumi.String("public.ecr.aws/aws-cli/aws-cli"),
